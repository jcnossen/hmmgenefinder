function [pStates,pSeq, fs, bs, s] = my_hmmdecode(seq,tr,e,varargin)
...
from = zeros(numStates);
fromC = zeros(1, numStates);

% Compute adjacency lists
for state = 1:numStates
    tmp = find(tr(:, state) > 0);
    toC(state) = length(tmp);
    to(state, 1:toC(state)) = tmp;
    
    tmp = find(tr(state, :) > 0);
    fromC(state) = length(tmp);
    from(state, 1:fromC(state)) = tmp;
end
...
for count = 2:L
    for state = 1:numStates
        ind = to(state, 1:toC(state));
        fs(state, count) = e(state, seq(count)) .* (sum(fs(ind, count - 1) .* tr(ind, state)));
    end
...
for count = L-1:-1:1
    for state = 1:numStates
        ind = from(state, 1:fromC(state));
        bs(state,count) = (1/s(count+1)) * sum(tr(state, ind)' .* bs(ind, count + 1) .* e(ind, seq(count + 1))); 
    end
end
