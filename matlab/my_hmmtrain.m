function [guessTR,guessE,logliks] = my_hmmtrain(seqs,guessTR,guessE,varargin)
...
%   $Revision: 1.4.4.9 $  $Date: 2007/10/15 23:37:03 $
...
for count = 1:numSeqs
    ...
    for j = 1:seqLength
        positions{count, seq(j)} = [positions{count, seq(j)}, j + 1];
    end
end
...
for iteration = 1:maxiter
    ...
    for count = 1:numSeqs
        ...
        if baumwelch   % Baum-Welch training
            ...
            [p, logPseq, fs, bs, scale] = my_hmmdecode(seq,guessTR,guessE);
            ...
            for i = 1:seqLength
                for l = 1:numStates
                    if (guessE(l, seq(i + 1)) > 0 || l == 1) % can we precompute these as well? Or do they change with time?
                        for pos = 1:toC(l)
                            k = to(l, pos);
